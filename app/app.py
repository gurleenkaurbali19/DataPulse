...
from utils.database_retrieve_operations import retrieve_table


# ---------- PAGE CONFIG ----------
st.set_page_config(page_title="DataPulse Entry Portal", layout="centered")
st.title("📊 DataPulse – Raw Data Entry Portal")
...
customers = retrieve_table("customers_raw") or []
products = retrieve_table("products_raw") or []
...
orders = retrieve_table("orders_raw") or []
customers = retrieve_table("customers_raw") or []

if not orders or not customers:
    st.warning("⚠️ Please add at least one order and customer first.")
else:
    cust_map = {c["customer_id"]: c["customer_name"] for c in customers}
    order_options = {
        f"{cust_map.get(o['customer_id'], 'Unknown')} (Order #{o['order_id']})": o["order_id"]
        for o in orders
    }
    ...
    products = retrieve_table("products_raw") or []
    selected_product = next(p for p in products if p["product_id"] == selected_order["product_id"])
...
# ---------- VIEW TABLES ----------
st.subheader("📋 View Raw Tables")

tables = ["customers_raw", "products_raw", "orders_raw", "sales_raw"]
selected_table = st.selectbox("Select Table to View", options=tables)
...
if st.button("Show Table Data"):
    ...
    if data:
        ...
        st.info(f"No data found in '{selected_table}' or failed to fetch.")
